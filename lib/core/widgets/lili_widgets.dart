...
/// Último recurso visual — nunca lança exceção de asset.
class _MascotIconFallback extends StatelessWidget {
  const _MascotIconFallback({required this.height, required this.label});
  final double height;
  final String label;

  @override
  Widget build(BuildContext context) {
    final size = height.clamp(40.0, 160.0);
    return Semantics(
      label: label,
      child: SizedBox(
        height: height,

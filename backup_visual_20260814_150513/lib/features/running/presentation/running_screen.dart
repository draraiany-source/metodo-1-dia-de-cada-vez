...
///
/// Usa o pacote `geolocator` para rastrear posição em tempo real,
/// calculando distância acumulada, ritmo médio e calorias estimadas.
///
/// OBS: o mapa visual (google_maps_flutter) está preparado no pubspec
/// mas comentado — habilite ao configurar a chave da Google Maps API.
...
class _RunningScreenState extends ConsumerState<RunningScreen> {
  ...
  Future<void> _start() async {
    ...
    if (!mounted) return; // usuária saiu da tela durante o diálogo de permissão
    if (!ok) {
    ...
    // `high` (≈10m) em vez de `best`: precisão suficiente para corrida e
    ...
    _posSub = Geolocator.getPositionStream(locationSettings: settings).listen(
      (pos) {
        ...
        if (_lastPosition != null) {
          ...
          // Descarta saltos de GPS (> 100 m em 1 update) que inflam a distância.
          if (meters < 100) {
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
                'Sessão será salva no seu histórico de corridas.',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Corrida Online 🏃')),
      body: Padding(
        padding: const EdgeInsets.all(20),
        child: Column(
          children: [
            // A mascote corre junto enquanto a corrida está ativa.
            AnimatedLiliMascot(
              pose: MascotePose.forte,
              mood: _running ? LiliMood.correndo : LiliMood.respirando,
              height: 96,
            ),
            const SizedBox(height: 8),
            // Área "mapa" (placeholder até configurar Google Maps).
            Expanded(
              child: Container(

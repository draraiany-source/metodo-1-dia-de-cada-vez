...
class HealthSyncNotifier extends StateNotifier<HealthSyncState> {
  ...
  Future<void> _persist() async {
    final p = await SharedPreferences.getInstance();
    await p.setBool(_kAutoSync, state.autoSync);
    if (state.lastSync != null) {
      await p.setString(_kLastSync, state.lastSync!.toIso8601String());
    }
    if (state.snapshot != null) {
      await p.setString(_kSnapshot, jsonEncode(state.snapshot!.toMap()));
    }
  }

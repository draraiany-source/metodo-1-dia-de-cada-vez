# -*- coding: utf-8 -*-
"""Gera PDF da homologaÃ§Ã£o pÃ³s-correÃ§Ã£o e tenta abrir."""
...
md_path = ROOT / "AUDITORIA_FINAL_METODO_1_DIA_PARA_PROGRAMADOR.md"
pdf_path = ROOT / "AUDITORIA_FINAL_METODO_1_DIA_PARA_PROGRAMADOR.pdf"
...
story.append(Paragraph("HOMOLOGAÃ‡ÃƒO PÃ“S-CORREÃ‡ÃƒO", styles["CoverTitle"]))
story.append(
    Paragraph(
        "MÃ©todo 1 Dia de Cada Vez â€” RelatÃ³rio ao Programador", styles["CoverSub"]
    )
)
story.append(
    Paragraph(
        "24/09/2026 Â· Sem senhas Â· Sem publicaÃ§Ã£o Â· Sem cobranÃ§a real",
        styles["CoverSub"],
    )
)
story.append(
...
story.append(
    Paragraph(
        "AMBIENTE WEB ATUALIZADO Â· E2E MANUAL NÃƒO EXECUTADO Â· PUBLICAÃ‡ÃƒO BLOQUEADA",
        styles["Verdict"],
    )
)
...
rows = [
    ...
    [
        Paragraph("PublicaÃ§Ã£o iOS", styles["Cell"]),
        Paragraph("<b>BLOQUEADA</b> (REPLACE_ME)", styles["Cell"]),
    ],
    [
        Paragraph("PublicaÃ§Ã£o Play", styles["Cell"]),
...
story.append(
    Paragraph(
        "Branch <b>finalizacao-metodo-1-dia-de-cada-vez</b> Â· HEAD "
        "<font face='Courier' size='8'>293ffa9</font> Â· working tree <b>sujo</b> "
        "(~1003) com correÃ§Ãµes nÃ£o commitadas. Carimbo pretendido: "
...
while i < len(lines):
    ...
    if line.startswith("### "):
        ...
        continue
    if re.match(r"^[-*] ", line):
        story.append(Paragraph("â€¢ " + inline(line[2:]), styles["BulletItem"]))
        i += 1
        continue
...
def on_page(canvas, doc):
    ...
    canvas.drawString(
        1.2 * cm,
        1 * cm,
        "MÃ©todo 1 Dia â€” HomologaÃ§Ã£o pÃ³s-correÃ§Ã£o 24/09/2026 â€” Sem senhas",
    )
    canvas.drawRightString(A4[0] - 1.2 * cm, 1 * cm, f"PÃ¡g. {doc.page}")
    canvas.restoreState()
...
doc = SimpleDocTemplate(
    ...
    topMargin=1.3 * cm,
    bottomMargin=1.5 * cm,
    title="HomologaÃ§Ã£o PÃ³s-CorreÃ§Ã£o â€” MÃ©todo 1 Dia",
    author="Programador",
)
